<template>
  <div class="flex flex-col items-center min-h-screen bg-gray-100 p-4">
    <h1 class="text-3xl font-bold mb-6 text-gray-800">
      Mastermind - Modalità Multiplayer
    </h1>

    <!-- Messaggio di Turno -->
    <p v-if="turn === 'setup'" class="text-lg text-gray-700">
      È il turno di Giocatore 1: Seleziona il codice segreto
    </p>
    <p v-if="turn === 'guess'" class="text-lg text-gray-700">
      È il turno di Giocatore 2: Prova a indovinare la sequenza
    </p>
    <p v-if="turn === 'feedback'" class="text-lg text-gray-700">
      È il turno di Giocatore 1: Fornisci il feedback
    </p>

    <!-- Selezione Modalità -->
    <div v-if="turn === 'modeSelection'" class="flex gap-4 mt-4">
      <button
        @click="selectMode(5, 15)"
        class="btn-mode bg-blue-600 text-white"
      >
        Modalità 5 Colori - 15 Tentativi
      </button>
      <button
        @click="selectMode(8, 25)"
        class="btn-mode bg-green-600 text-white"
      >
    ...
    <div
      v-if="turn === 'victory'"
      class="text-4xl font-bold text-green-500 victory-animation mt-8"
    >
    ...
    <button
      v-if="
        currentAttempt.length === 4 && (turn === 'setup' || turn === 'guess')
      "
      @click="submitTurn"
    ...
    <div class="flex gap-4 mt-6">
      <button @click="restartGame" class="btn-action bg-red-600 text-white">
        Ricomincia
      </button>
      <NuxtLink to="/" class="btn-action bg-gray-600 text-white"
        >Torna alla Home</NuxtLink
      >
      <!-- Pulsante per mostrare il regolamento -->
      <button @click="toggleInfo" class="btn-action bg-blue-500 text-white">
        Mostra Regolamento
      </button>
    </div>
    ...
    <p v-if="errorMessage" class="text-red-500 font-semibold mt-4">
      {{ errorMessage }}
    </p>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const turn = ref<
      "modeSelection" | "setup" | "guess" | "feedback" | "victory" | "defeat"
    >("modeSelection");
